...
\title{``거의 정수(almost integer)''가 되는 $(a + b\sqrt{d})$의 조건}
\date{}
\maketitle

\section*{\emoji{keycap-1} 문제 설정}
...
이때 $\alpha^n$이 어떤 정수에 \textbf{지수적으로 가까워지는지} 알고 싶어요.
핵심은 다음 관계식입니다:
...
또한 $|\beta| < \frac{1}{2}$이면 \textbf{모든 $n$}에서 $\alpha^n$의 반올림이 정확히 $k_n$이 됩니다.
($|\beta|^n < \frac{1}{2}$일 때마다 반올림이 정확해지기 때문이에요.)

\section*{\emoji{keycap-3} 왜 $k_n$이 정수인가?}
...
\section*{\emoji{keycap-4} 점화식으로 $k_n$ 만들기}
...
\section*{\emoji{keycap-5} Pell 방정식과의 연결}
...
이게 바로 \textbf{거의 정수(almost integer)} 현상이에요.

\section*{\emoji{keycap-6} 왜 $1.98$은 안 되는가?}

\begin{itemize}
\item $1.98 = \frac{99}{50}$은 \textbf{유리수},
  즉 어떤 $\sqrt{d}$와의 켤레 구조가 없음.
\item $\alpha^n = (99/50)^n = 99^n / 50^n$ 은 단순한 분수 형태이고
  분모가 절대 약분되지 않으므로 정수화될 수 없음.
\item 소수부 $(1.98)^n \bmod 1$은 \textbf{거의 균등분포(equidistributed)} 로 퍼짐.
  정수 근접은 단지 ``우연''일 뿐 구조적 이유가 없습니다.
\end{itemize}

\section*{\emoji{keycap-7} 요약}

\begin{enumerate}
\item $\alpha = a + b\sqrt{d}$가 정수에 근접하려면
   $\alpha\beta = \pm 1,\ \alpha > 1,\ |\beta| < 1.$
\item 이 조건을 만족하면 $\alpha^n + \beta^n$은 항상 정수이고,
   $\alpha^n$은 그 정수에 지수적으로 가까움.
\item 특히 $|\beta| < 1/2$이면 \textbf{모든 n}에서 반올림이 정확함.
\item 유리수(예: 1.98)는 이런 구조가 없으므로 ``almost integer''가 될 수 없음.
\end{enumerate}

\section*{\emoji{keycap-8} 예시}
...
\hline
\textbf{예시} & \textbf{$d$} & \textbf{$N(\alpha)$} & \textbf{$|\beta|$} & \textbf{almost integer 성질} \\
\hline
...
\alpha^n=k_n-\beta^n,\quad |\alpha^n-k_n|=|\beta|^n
\]
을 실제로 ``보여줄''게. 아래 수들 모두 $|\beta|<\tfrac{1}{2}$라서 \textbf{모든 $n\ge1$}에서 $\mathrm{round}(\alpha^n)=k_n$가 성립한다.

\subsection*{예시 1) $d=2,\ \alpha=3+2\sqrt{2}$}

\[
a=3,\ b=2,\ \beta=3-2\sqrt{2}\approx 0.17157288,\quad
...
\subsection*{예시 2) $d=3,\ \alpha=2+\sqrt{3}$}

\[
a=2,\ b=1,\ \beta=2-\sqrt{3}\approx 0.26794919,\quad
...
\subsection*{예시 3) $d=6,\ \alpha=5+2\sqrt{6}$}

\[
a=5,\ b=2,\ \beta=5-2\sqrt{6}\approx 0.10102051,\quad
...
\subsection*{예시 4) $d=37,\ \alpha=6+\sqrt{37}$}
...
\begin{itemize}
\item 위 네 케이스는 모두 $N(\alpha)=a^2-db^2=\pm1$ (단위)이고 $\alpha>1\Rightarrow |\beta|<1$.
\item 따라서 $k_n=\alpha^n+\beta^n\in\mathbb{Z}$이며, $\alpha^n$의 최근접 정수는 $k_n$.
\item 오차는 정확히 $|\beta|^n$로 \textbf{지수적}으로 감소. 위 표의 $|\beta|^n$ 열이 그 수치다.
\end{itemize}

\vspace{1em}

원하면 특정 $d$를 지정해서, 그 $d$의 \textbf{기본 단위}(Pell 해)부터 잡고 $k_n$과 $|\beta|^n$을 더 길게 $n$까지 전개해 줄게.
